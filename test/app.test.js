...
describe('routes', () => {
  ...
  test('GET / exposes the student-editable version values', async () => {
    const res = await request(app).get('/');
    // These come from src/version.js — the file students edit
    expect(res.body).toHaveProperty('releaseName');
    expect(res.body).toHaveProperty('buildMessage');
    expect(typeof res.body.releaseName).toBe('string');
  });
